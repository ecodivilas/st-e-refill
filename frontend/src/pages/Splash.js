  ...
  return (
    <>
      ...
      <div className="flex justify-center group relative py-4 px-2">
        <div className="mt-32">
          <a href="/" className="" data-tooltip-target="tooltip-hover">
            <img className="h-4/6 ml-6 z-10 translate-x-6 hover:translate-x-full duration-150" src={require("../assets/img/robots.webp")} alt="robot" />
          </a>
      ...
      <div className="flex flex-col justify-end items-center w-5/12 h-28 gap-3">
          ...
          <button type="button" onClick={() => {isAuthorized ? navigate('/order') : navigate('/login')}}
          className="text-white bg-slate-900 hover:bg-orange-500 focus:ring-4 focus:bg-slate-500 font-medium text-lg h-14 !w-40">Order Now</button>
       {isAuthorized ? (
         <div></div>
          ) : ( 
