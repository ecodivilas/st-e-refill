...
function OrderSchedule() {
    ...
    const handleNext = () => {
        ...
        localStorage.setItem(
            'dateValue',
            updatedSchedule.deliveryDate.toString()
        )
        localStorage.setItem(
            'timeValue',
            updatedSchedule.deliveryTime.toString()
        )
    ...
    return (
        <div className="w-[100vw] h-[100vh] flex bg-slate-100 justify-center items-start pt-20">
            <div className="flex-col gap-2">
                <div className="py-7 font-semibold text-center text-2xl">
                    When do you want to deliver your order?
                </div>
                ...
                {isProceed && <Navigate to="/order-mode-of-payment" />}
            </div>
        </div>
    )
}
