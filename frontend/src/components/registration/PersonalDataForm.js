            ...
            <div className="grid grid-cols-2 gap-4">
                {registerFields.map((item) => (
                    <div key={item.id} className="h-16">
                        ...
                        {item.id === 'gender' ? (
                            <select name="gender" defaultValue={defaultUserValues[item.name]} onChange={handleUserChange} className="px-2 mt-2 h-8 bg-gray-50 border border-gray-300 text-gray-900 sm:text-xs rounded-sm block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white w-72">
                                <option value=""></option>
