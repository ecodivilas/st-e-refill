...
useEffect(() => {
    ...
    const statusChecker = (status) => {
        if (status === "pick up") {
            return <button className='py-1 w-20 text-xm rounded-lg text-black font-medium bg-yellow-400'>Pick Up</button>
        }
        else if (status === "cancelled") {
            return <button className='py-1 w-20 text-xm rounded-lg text-white font-medium bg-gray-600'>Cancelled</button>
        }
        else if (status === "preparing") {
            return <button className='py-1 w-20 text-xm rounded-lg text-white font-medium bg-teal-900'>Preparing</button>
        }
        else if (status === "on the way") {
            return <button className='py-1 w-20 text-xm rounded-lg text-white font-medium bg-orange-600'>On the Way</button>
        }
        else if (status === "delivered") {
            return <button className='py-1 w-20 text-xm rounded-lg text-white font-medium bg-lime-600'>Delivered</button>
        }
    }
